...
def compute_lidar_angle(lidar_track):
    ...
    Funkcja wykorzystuje funkcję atan2 do obliczenia kąta w stopniach między
    osią przednią (y) a pozycją obiektu (x, y). Dodatni kąt oznacza odchylenie
    w lewo, ujemny - w prawo.
    
    Argumenty:
        lidar_track (dict): Słownik zawierający dane śledzenia LIDAR, musi zawierać
            klucz "last_position" z listą [x, y], gdzie x to odchylenie boczne,
            a y to odległość do przodu w metrach.
    
    Zwraca:
        float: Kąt w stopniach. Wartość dodatnia = lewo, ujemna = prawo.
    
    Hierarchia wywołań:
        consolidator.py (główna pętla while) -> compute_lidar_angle()
    """
...
def main():
    """
    Główna pętla konsolidatora danych.
    
    Łączy dane z Lidara (lidar.json) i Kamery (camera.json) w jeden spójny format,
    dopasowując obiekty na podstawie kąta widzenia. Wynik zapisuje do consolidator.json.
    
    Hierarchia wywołań:
        consolidator.py -> main()
    """
    while True:
